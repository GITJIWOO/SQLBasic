...
아래 구문은 좌측 SCHEMAS에서 employees를 우클릭 한 후
...
-- 데이터베이스 삭제 및 생성
-- 보통 DB는 그냥 생성하지만
-- 간혹 DB가 있는데 초기화 하기위해 만약 기존에 같은 이름을 가진
...
-- 따라서 조건에 맞도록 필터링을 할 수 있고, 이를 위해 시용하는 구문은
-- WHERE구문입니다.
...
-- and, or를 이용해 조건 여러개를 연결할 수 있습니다.
-- 1970년 이후 출생했으며, 키가 182 이상인 사람의 아이디와 이름을
-- 조회하는 구문을 아래에 작성해주세요.(workbencg, 커맨드라인 둘 다)
...
-- 16. 서브쿼리
-- 서브쿼리(하위쿼리)란 1자식 결과를 얻어보고. 거기서 다시
...
-- 조건으로 들어갑니다.(개별 값 하나하나에 대해 OR이 걸림)
-- 즉 170보다 크거나 같다고 결론이 나옵니다.
SELECT * FROM userTbl WHERE height >= ANY (
	SELECT height FROM userTbl WHERE addr = '경남'
);

-- ALL 구문을 사용하면 (170보다 크거나 같은) AND (173보다 크거나 같은)
-- 조건으로 들어갑니다.(개별 조건 하나하나에 대해 AND가 걸림)
...
-- 
-- 18. ORDER BY (ASC, DESC) 구문
-- ORDER BY는 결과물의 개수나 종류에는 영향을 미치지 않지만
-- 결과물을 순서대로(오름차순, 내림차순) 정령하는 기능을 가집니다.
-- SELECT 컬럼명 FROM 테이블명 ORDER BY 기준컬럼 정령기준;
-- 정렬기준은 ASC(오름차순), DESC(내림차순)이 있으며 입력이 없을 경우 기본은 ASC입니다.
-- 가입한 순서(mDate 수능으로 오름차순 조회를 해주세요.)
SELECT * FROM userTbl ORDER BY mDate ASC;

-- 정렬시 동점인 부분을 처리하기 위해 order by 절을 두 개 이상
-- 동시에 나열할 수도 있습니다.
...
-- employees 안에 gender는 몇종류인지 출력
SELECT DISTINCT gender FROM employees;

-- 20. LIMIT 구문
-- 출력 요소의 개수를 제한할 떄는 LIMIT 구문을 사용합니다.
use employees;

-- 데이터가 300000개에 가깝다 보니 모든 데이터를 보여주지 않고
-- 1000개만 보여주도록 설정되어 있음.
SELECT * FROM employees;

-- 테이블명 뒤에 LIMIT 숫자; 가 오는 경우는 적은 숫자 개수만큼만
-- 결과창에 보여줍니다.
-- 하남나 적으면 자동으로 0번부터 n개를 보여줍니다.
SELECT * FROM employees LIMIT 10;

-- 만약에 LIMIT 숫자1, 숫자2; 와 같이 숫자 2개를 넣는 경우
-- 숫자 1번부터 숫자 2개를 부여줍니다.
SELECT * FROM employees LIMIT 5, 10; -- 5번째부터 15번째 까지(0부터 시작)
...
-- 각 물품구룹별 총 판매금액을 집계해주세요.
-- 22. as 문
-- 컬럼명은 as '변경명칭' 으로 변경할 수 있습니다.
SELECT groupName, sum(price * amount) as '총 매출' FROM buyTbl GROUP BY groupName;
...
SELECT name, height as '키' FROM userTbl WHERE height = 
